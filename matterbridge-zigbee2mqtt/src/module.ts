...
export class ZigbeePlatform extends MatterbridgeDynamicPlatform {
  ...
  public z2m: Zigbee2MQTT | Zigbee2MQTTWs;
  ...
  constructor(
    ...
    // Load transport mode
    this.transport = config.transport === 'mqtt' ? 'mqtt' : 'websocket';

    // Default values and validation
    if (config.host && typeof config.host === 'string') {
      this.wsHost = config.host;
    }
    if (config.port) this.wsPort = config.port;
    if (config.topic) this.mqttTopic = config.topic;
    if (config.token) this.wsToken = config.token;
    if (config.mqttHost && typeof config.mqttHost === 'string') {
      this.mqttHost = config.mqttHost;
    }
    if (config.mqttPort) this.mqttPort = config.mqttPort;
    if (config.mqttUsername) this.mqttUsername = config.mqttUsername;
    if (config.mqttPassword) this.mqttPassword = config.mqttPassword;
    ...
    config.host = this.wsHost;
    config.port = this.wsPort;
    config.topic = this.mqttTopic;
    config.token = this.wsToken ?? '';
    config.mqttHost = this.mqttHost;
    config.mqttPort = this.mqttPort;
    config.mqttUsername = this.mqttUsername ?? '';
    config.mqttPassword = this.mqttPassword ?? '';
    ...
    this.log.info(`Transport mode: ${CYAN}${this.transport}${rs}`);
    // this.log.debug(`Config:\n${rs}${JSON.stringify(config, null, 2)}${rs}`);

    if (this.transport === 'websocket') {
      this.z2m = new Zigbee2MQTTWs(
        this.wsHost,
        this.wsPort,
        this.mqttTopic,
        this.wsToken,
      ...
      if (isValidString(this.wsHost) && isValidNumber(this.wsPort, 1, 65535)) {
        this.log.info(`Connecting to Zigbee2MQTT frontend WebSocket: ${this.z2m.getUrl()}`);
        this.z2m.start();
      } else {
        this.log.error(`Invalid Zigbee2MQTT host: ${this.wsHost} or port: ${this.wsPort}`);
      }

      this.z2m.on('ws_connected', () => {
        this.log.info(`WebSocket connected to Zigbee2MQTT frontend at ${this.z2m.getUrl()}`);
      });

      this.z2m.on('ws_close', () => {
        this.log.warn(`WebSocket connection to ${this.z2m.getUrl()} closed`);
      });

      this.z2m.on('ws_reconnect', () => {
        this.log.warn(`WebSocket reconnecting to ${this.z2m.getUrl()}...`);
      });

      this.z2m.on('ws_error', (error: unknown) => {
        this.log.error(`WebSocket error at ${this.z2m.getUrl()}:`, error);
      });
    } else {
