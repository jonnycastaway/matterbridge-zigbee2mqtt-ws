...
import { AnsiLogger, db, dn, er, gn, hk, id, idn, ign, LogLevel, rs, TimestampFormat, zb } from 'node-ansi-logger';
...
export class Zigbee2MQTTWs extends EventEmitter {
  ...
  private wsIsReconnecting = false;
  private wsIsEnding = false;
  private wsReconnectTimer: NodeJS.Timeout | undefined;
  private wsDataPath = '';
  private wsPublishQueue: PublishQueue[] = [];
  private wsPublishQueueTimeout: NodeJS.Timeout | undefined;
  private wsPublishInflights = 0;
  ...
  constructor(
    ...
    this.log.debug(
      `Created new instance with wsHost: ${wsHost} wsPort: ${wsPort} topic: ${mqttTopic} token: ${wsToken !== undefined && wsToken !== '' ? '***' : 'undefined'}`,
    );
  }
  ...
  private connectWs(): void {
    ...
    this.wsClient.on('message', (data: Buffer) => {
      try {
        const msg = JSON.parse(data.toString()) as { topic: string; payload: unknown };
    ...
    const doSend = (): void => {
      if (this.wsClient && this.wsIsConnected && this.wsClient.readyState === WebSocket.OPEN) {
        ...
        this.wsClient.send(wsMsg, (err) => {
          if (err) this.log.error(`WebSocket send error: ${err.message}`);
          this.wsPublishInflights--;
          if (!err) this.emit('ws_published');
        });
      } else {
        ...
        this.wsPublishQueueTimeout = setInterval(() => {
          if (this.wsPublishQueue.length > 0) {
            ...
            if (this.wsClient && this.wsIsConnected) {
              ...
              const p = (() => { try { return JSON.parse(item.message); } catch { return item.message; } })();
              this.wsClient!.send(JSON.stringify({ topic: t, payload: p }), (err) => {
                if (err) this.log.error(`WebSocket queue send error: ${err.message}`);
              });
            }
          } else {
            clearInterval(this.wsPublishQueueTimeout!);
  ...
  private writeBufferJSON(file: string, buffer: Buffer): void {
    ...
    fs.promises
      ...
      .catch((error: unknown) => this.log.error(`Error writing to ${filePath}.json:`, error));
  }
  ...
  private writeFile(file: string, data: string): void {
    ...
    fs.promises
      ...
      .catch((error: unknown) => this.log.error(`Error writing to ${filePath}:`, error));
  }
  ...
  private handleResponseNetworkmap(payload: Buffer): void {
    ...
    const lqi = (lqi: number): string => {
      if (lqi < 50) return `\x1b[31m${lqi.toString().padStart(3, ' ')}${db}`;
      else if (lqi > 200) return `\x1b[32m${lqi.toString().padStart(3, ' ')}${db}`;
      else return `\x1b[38;5;251m${lqi.toString().padStart(3, ' ')}${db}`;
    };
    const depth = (depth: number): string => {
      if (depth === 255) return `\x1b[32m${depth.toString().padStart(3, ' ')}${db}`;
      else return `\x1b[38;5;251m${depth.toString().padStart(3, ' ')}${db}`;
    };
    const relationship = (relationship: number): string => {
      if (relationship === 0) return `${zb}${relationship}-IsParent  ${db}`;
      else if (relationship === 1) return `${hk}${relationship}-IsAChild  ${db}`;
      else return `${relationship}-IsASibling`;
    };
    const friendlyName = (ieeeAddr: string): string => {
      const node = topology.nodes.find((node) => node.ieeeAddr === ieeeAddr);
      if (node) {
        if (node.type === 'Coordinator') return `\x1b[48;5;1m\x1b[38;5;255m${node.friendlyName} [C]${rs}${db}`;
        else if (node.type === 'Router') return `${dn}${node.friendlyName} [R]${db}`;
        else if (node.type === 'EndDevice') return `${gn}${node.friendlyName} [E]${db}`;
      }
