...
export class ZigbeeEntity extends EventEmitter {
    ...
    constructor(platform, entity) {
        ...
        this.log = new AnsiLogger({
            ...
            logLevel: platform.config.debug ? "debug" /* LogLevel.DEBUG */ : platform.log.logLevel,
        });
        ...
        this.platform.z2m.on('MESSAGE-' + this.entityName, (payload) => {
            ...
            Object.entries(payload).forEach(([key, value]) => {
                ...
                if (value === undefined || value === null)
                    return;
                if (this.bridgedDevice === undefined)
                    return; // Typescript only check for undefined but we checked before
                ...
                if (key === 'action' && typeof value === 'string' && value !== '') {
                    ...
                    if (propertyMap) {
                        ...
                        if (child?.maybeNumber)
                            ...
                            child.triggerSwitchEvent(propertyMap.action, this.log), this.log, `Error triggering switch event ${propertyMap.action} on ${propertyMap.endpoint}`);
                    }
                    else
    ...
    updateAttributeIfChanged(deviceEndpoint, childEndpointName, clusterId, attributeName, value, lookup) {
        ...
        fireAndForget(deviceEndpoint.setAttribute(ClusterId(clusterId), attributeName, value), this.log, 
...
export class ZigbeeGroup extends ZigbeeEntity {
    ...
    static async create(platform, group) {
        ...
        if (group.members.length === 0) {
            ...
            zigbeeGroup.bridgedDevice = new MatterbridgeEndpoint([onOffLightSwitch, bridgedNode, powerSource], { id: group.friendly_name }, zigbeeGroup.log.logLevel === "debug" /* LogLevel.DEBUG */);
            ...
            if (!deviceType)
                return zigbeeGroup;
            zigbeeGroup.bridgedDevice = new MatterbridgeEndpoint([deviceType, bridgedNode, powerSource], { id: group.friendly_name }, zigbeeGroup.log.logLevel === "debug" /* LogLevel.DEBUG */);
...
export class ZigbeeDevice extends ZigbeeEntity {
    ...
    static async create(platform, device) {
        ...
        if (device.friendly_name === 'Coordinator' ||
            ...
            zigbeeDevice.bridgedDevice = new MatterbridgeEndpoint([doorLock, bridgedNode, powerSource], { id: device.friendly_name }, zigbeeDevice.log.logLevel === "debug" /* LogLevel.DEBUG */);
        ...
        */
        for (const [index, name] of names.entries()) {
            ...
            if (name === 'action' && zigbeeDevice.actions.length) {
                ...
                zigbeeDevice.bridgedDevice ??= new MatterbridgeEndpoint([bridgedNode], { id: device.friendly_name }, zigbeeDevice.log.logLevel === "debug" /* LogLevel.DEBUG */);
                zigbeeDevice.hasEndpoints = true;
                // Mapping actions
        ...
        // oxlint-disable-next-line typescript/no-unsafe-type-assertion
        [...mainEndpoint.deviceTypes, bridgedNode, powerSource], { id: device.friendly_name }, zigbeeDevice.log.logLevel === "debug" /* LogLevel.DEBUG */);
        ...
        if (mainEndpoint.clusterServersIds.includes(Thermostat.id)) {
            ...
            if ((heat && !cool) || (!system_mode_values?.includes('auto') && system_mode_values?.includes('heat'))) {
                ...
                if (sMode)
                    sMode.values = 'off|heat'; // Set system_mode to off|heat if only heating is supported cause it's not supported by the cluster without AutoMode
            }
                ...
                const sMode = zigbeeDevice.propertyMap.get('system_mode');
                if (sMode)
                    sMode.values = 'off|cool'; // Set system_mode to off|cool if only cooling is supported cause it's not supported by the cluster without AutoMode
            }
            else {
        ...
        for (const [endpoint, device] of zigbeeDevice.mutableDevice) {
            if (endpoint === '')
                continue;
            zigbeeDevice.bridgedDevice?.addChildDeviceTypeWithClusterServer(endpoint, 
            // oxlint-disable-next-line typescript/no-unsafe-type-assertion
            device.deviceTypes, device.clusterServersIds, { tagList: device.tagList }, zigbeeDevice.log.logLevel === "debug" /* LogLevel.DEBUG */);
        }
        // Verify the device
