...
export class ZigbeePlatform extends MatterbridgeDynamicPlatform {
    ...
    constructor(matterbridge, log, config) {
        ...
        if (this.transport === 'websocket') {
            this.z2m = new Zigbee2MQTTWs(this.wsHost, this.wsPort, this.mqttTopic, this.wsToken, config.debug);
            ...
            this.z2m.on('ws_error', (error) => {
                this.log.error(`WebSocket error at ${this.z2m.getUrl()}:`, error);
            });
        }
        else {
            this.z2m = new Zigbee2MQTT(this.mqttHost, this.mqttPort, this.mqttTopic, this.mqttUsername, this.mqttPassword, this.config.clientId, this.config.protocolVersion || 5, this.config.ca, this.config.rejectUnauthorized, this.config.cert, this.config.key, config.debug);
            this.z2m.setLogDebug(config.debug);
            this.z2m.setDataPath(path.join(matterbridge.matterbridgePluginDirectory, 'matterbridge-zigbee2mqtt'));
